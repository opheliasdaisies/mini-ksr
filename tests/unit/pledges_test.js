...
describe('Backers are able to contribute to a project.', function() {
  ...
  it ('Should return a promise that resolves to an error if credit card numbers are longer than 19 characters.', function() {
    var pledgePromise = pledge.backProject('Ida_Backer', 'Super-Project', '12121941847283719261249286e12', 1000);
    var expectedError = 'Credit card numbers can not be longer than 19 characters.';
    return promiseIsExpectedError(pledgePromise, expectedError);
  });

  it ('Should return a promise that resolves to an error if credit card numbers are not numeric.', function() {
    var pledgePromise = pledge.backProject('Ida_Backer', 'Super-Project', '871hi8a02', 1000);
    var expectedError = 'Credit cards must contain only numeric characters.';
    return promiseIsExpectedError(pledgePromise, expectedError);
  });

  it ('Should return a promise that resolves to an error if credit card numbers do not pass Luhn-10 validation.');

  it ('Should return a promise that resolves to an error if the credit card number entered has already been used.');

  it ('Should have a pledge value that accepts dollars and cents.');

  it ('Should return a promise that resolves to an error if the target is preceeded with $');
